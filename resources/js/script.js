...
img.src = url; // If both image and site is ready to show:
...
if (window.innerWidth >= 900) {
  if (window.scrollY < 500) elements.popUpNavIcon.css({
    'display': 'none'
  });
  ...
  if (window.scrollY < 200) elements.popUpNavIcon.css({
...
window.addEventListener('resize', function () {
  if (window.innerWidth >= 900) {
    if (window.scrollY < 500) elements.popUpNavIcon.css({
      'display': 'none'
    });else elements.popUpNavIcon.css({
      'display': 'block'
    });
  } else {
    elements.popUpNavIcon.css({
